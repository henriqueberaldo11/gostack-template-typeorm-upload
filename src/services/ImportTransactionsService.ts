...
class ImportTransactionsService {
  async execute(filePathCSV: string): Promise<Transaction[]> {
    const transactionRepository = getCustomRepository(TransactionRepository);
    const categoriesRepository = getRepository(Category);

    const readFileStream = createReadStream(filePathCSV);
    ...
    access(filePathCSV, async error => {
      if (!error) {
        await promises.unlink(filePathCSV);
      }
    });
    return transactions;
  }
